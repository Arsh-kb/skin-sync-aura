...
export function ProductCard({ product, className, onClick, variant = "default" }: ProductCardProps) {
  return (
    <div
      onClick={onClick}
      className={cn("glass rounded-2xl overflow-hidden hover-lift cursor-pointer group relative", className)}
    >
            ...
            {product.ingredients.slice(0, 2).map((ing) => (
              <IngredientSheet key={ing.name} ingredientName={ing.name}>
                <button
                  ...
                  className="text-[8px] px-1.5 py-0.5 rounded-full bg-background/50 backdrop-blur-sm text-foreground font-medium hover:bg-primary/20 transition-colors"
                >
                  {ing.name}
                </button>
              </IngredientSheet>

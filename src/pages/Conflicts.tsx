  ...
  return (
    <div className="min-h-screen pb-24 md:pb-8">
      ...
      <div className="px-5 md:px-10 -mt-6 space-y-6 relative z-10">
        {/* Status summary */}
        <div className={cn("glass-strong rounded-2xl p-5 flex items-center gap-4 animate-slide-up", conflictCount > 0 && "animate-shake-once")}>
          <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center shrink-0", conflictCount > 0 ? "bg-conflict/15" : "bg-safe/15")}>
            {conflictCount > 0 ? <AlertOctagon size={24} className="text-conflict" /> : <Shield size={24} className="text-safe" />}
          </div>
          <div>
        ...
        {/* Conflict Examples with clickable ingredient names */}
          ...
          <div className="space-y-4">
            {conflictExamples.map((item, i) => {
              ...
              return (
                <div
                  key={item.pair}
                  className={cn("glass rounded-2xl p-5 space-y-3 hover-lift", i % 2 === 0 ? "md:ml-0 md:mr-12" : "md:ml-12 md:mr-0")}
                >

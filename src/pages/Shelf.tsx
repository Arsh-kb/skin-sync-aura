    ...
    <div className="min-h-screen pb-24 md:pb-8 relative overflow-hidden">
      ...
      <div className="px-5 md:px-10 space-y-5 mt-6">
        ...
        <div className="relative animate-slide-up" style={{ animationDelay: "0.05s" }}>
          ...
          <Input placeholder="Search products or ingredients..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-11 rounded-2xl border-border/40 bg-card/50 backdrop-blur-sm h-11 text-sm" />
        </div>
          ...
          <div className="space-y-3 animate-slide-up" style={{ animationDelay: "0.1s" }}>
            ...
            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-none -mx-1 px-1">
              {products.slice(0, 4).map((product, i) => (
                <div key={product.id} className={cn("shrink-0 w-44 md:w-52 rounded-2xl overflow-hidden hover-lift cursor-pointer group relative h-56 md:h-64", i % 2 === 0 ? "glass-rose" : "gradient-warm")}>
        ...
        {filtered.length > 0 ? (
          <div className="columns-2 md:columns-3 gap-3.5 space-y-3.5">
            {filtered.map((product, i) => (
              <div key={product.id} className="break-inside-avoid animate-slide-up" style={{ animationDelay: `${i * 0.06}s` }}>
          ...
          <div className="flex flex-col items-center justify-center py-20 space-y-4">
            <div className="w-20 h-20 rounded-full gradient-blush flex items-center justify-center"><Package size={30} className="text-primary/40" /></div>
            <h3 className="font-display font-semibold text-foreground text-lg">No products found</h3>
            <p className="text-sm text-muted-foreground text-center max-w-[220px]">Start building your shelf by adding your first product</p>
            <AddProductModal />
          </div>
        )}
      </div>
